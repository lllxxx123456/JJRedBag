...
// 会话激活且 jj_momentsVideoTargetWidth/Height 已从 cachedEditedAssetFileURL 读出真实尺寸时，
// 用真实尺寸替换微信传入的降级值（仅在传入值 <= 720 时替换，避免干扰正常的更大值）
- (void)setWidth:(float)width {
    ...
    if ([JJDebugConsole isEnabled] && jj_momentsOriginalPickerSessionPending) {
        JJ_LOG(@"视频", @"VideoEncodeParams.setWidth: %.0f 会话=YES (保留)", width);
    }
    %orig;
}
...
- (void)setHeight:(float)height {
    ...
    if ([JJDebugConsole isEnabled] && jj_momentsOriginalPickerSessionPending) {
        JJ_LOG(@"视频", @"VideoEncodeParams.setHeight: %.0f 会话=YES (保留)", height);
    }
    %orig;
}
    ...
    if (active && task) {
        @try {
            ...
            if ([urlObj isKindOfClass:[NSURL class]]) {
                ...
                // AVURLAsset 可以在方法体内使用（AVFoundation 在本文件前部已 import）
                ...
                if (tracks.count > 0) {
                    ...
                    // 应用变换后的展示尺寸（考虑旋转），结果取绝对值
                    CGRect box = CGRectApplyAffineTransform(CGRectMake(0, 0, natural.width, natural.height), tf);
        ...
        // 反射 dump srcPHAsset（MMAssetForPHAssetFramework）的所有属性，找真实尺寸字段名
        if ([JJDebugConsole isEnabled]) {
            @try {
                id srcPHAsset = nil;
                @try { srcPHAsset = [task valueForKey:@"srcPHAsset"]; } @catch (NSException *e) {}
                if (srcPHAsset) {
                    NSMutableString *sdump = [NSMutableString string];
                    Class scls = [srcPHAsset class];
                    int sdepth = 0;
                    while (scls && scls != [NSObject class] && sdepth < 3) {
                        unsigned int count = 0;
                        objc_property_t *props = class_copyPropertyList(scls, &count);
                        for (unsigned int i = 0; i < count; i++) {
                            const char *name = property_getName(props[i]);
                            NSString *propName = [NSString stringWithUTF8String:name];
                            id v = nil;
                            @try { v = [srcPHAsset valueForKey:propName]; } @catch (NSException *e) { continue; }
                            NSString *desc;
                            if (v == nil) {
                                desc = @"<nil>";
                            } else if ([v isKindOfClass:[NSString class]]) {
                                NSUInteger l = [(NSString *)v length];
                                desc = l > 60 ? [[(NSString *)v substringToIndex:57] stringByAppendingString:@"..."] : v;
                            } else if ([v isKindOfClass:[NSNumber class]]) {
                                desc = [(NSNumber *)v stringValue];
                            } else if ([v isKindOfClass:[NSURL class]]) {
                                desc = [(NSURL *)v absoluteString];
                            ...
                            [sdump appendFormat:@"%@=%@; ", propName, desc];
                        }
                        if (props) free(props);
                        scls = [scls superclass];
                        sdepth++;
                    }
                    JJ_LOG(@"视频", @"srcPHAsset(%@) 全属性: %@",
                           NSStringFromClass([srcPHAsset class]) ?: @"?", sdump);
                }
            } @catch (NSException *e) {}
        }
